...
<template>
  ...
  >
    <NTabPane
      name="top"
      :tab="t('browse.lists.tabs.top')"
    />
    <NTabPane
      name="categories"
      :tab="t('browse.lists.tabs.categories')"
    />
    <NTabPane
      name="tags"
      :tab="t('browse.lists.tabs.tags')"
    />
    <NTabPane
      name="custom"
      :tab="t('browse.lists.tabs.custom')"
    />
  </NTabs>
  ...
  >
    ...
    <NButton
      type="primary"
      :loading="browse.listsLoading"
      :disabled="!browse.listsUrl.trim()"
      @click="onLoad"
    >
      {{ t('browse.lists.load') }}
    </NButton>
  ...
  <NAlert
    v-if="browse.listsError"
    type="warning"
    :show-icon="true"
    style="margin-top: 8px;"
    closable
    @close="browse.listsError = null"
  >
