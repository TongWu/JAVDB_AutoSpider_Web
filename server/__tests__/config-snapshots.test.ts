...
beforeEach(async () => {
  ...
  await env.OPERATIONS_DB.prepare('CREATE TABLE IF NOT EXISTS api_config (key TEXT PRIMARY KEY, value TEXT, updated_at TEXT)').run();
...
describe('ADR-061 evidence', () => {
  ...
  it('never emits credentials, arbitrary JSON or unsafe scalar values', () => {
    for (const key of ['API_SECRET_KEY', 'ADMIN_PASSWORD_HASH', 'JAVDB_SESSION_COOKIE', 'PROXY_POOL', 'PROXY_HTTP', 'QB_URL', 'CLOUDFLARE_API_TOKEN', 'UNKNOWN_SETTING', 'JAVDB_USERNAME']) {
  ...
  it('retains the original snapshot after config edits and retries', async () => {
    await env.OPERATIONS_DB.prepare('INSERT INTO api_config (key,value) VALUES (?,?)').bind('PAGE_END', '7').run();
    ...
    await env.OPERATIONS_DB.prepare('UPDATE api_config SET value = ? WHERE key = ?').bind('99', 'PAGE_END').run();
    ...
    await expect(env.OPERATIONS_DB.prepare("UPDATE ConfigSnapshots SET digest = 'bad'").run()).rejects.toThrow('immutable');
  });

  it('hides and deletes evidence at the exact 90-day boundary', async () => {
    await snapshots.insertSnapshot(env.OPERATIONS_DB, 'job-old', golden.snapshot as snapshots.Snapshot);
    const expires = Date.parse(golden.snapshot.captured_at) / 1000 + 90 * 86400;
    expect((await snapshots.readJobSnapshots(env.OPERATIONS_DB, 'job-old', expires - 1)).consumers[0].status).toBe('captured');
    expect((await snapshots.readJobSnapshots(env.OPERATIONS_DB, 'job-old', expires)).consumers[0].status).toBe('snapshot_unavailable');
    await snapshots.cleanupSnapshots(env.OPERATIONS_DB, expires);
    expect((await snapshots.readJobSnapshots(env.OPERATIONS_DB, 'job-old', expires - 1)).consumers[0].status).toBe('snapshot_unavailable');
  });

  it('reports D1 failure without throwing or logging exception contents', async () => {
    const error = vi.spyOn(console, 'error').mockImplementation(() => {});
    const badEnv = {...env, OPERATIONS_DB: {prepare() {throw new Error('secret-canary');}} as unknown as D1Database};
  ...
  it('requires authentication and admin access for both read surfaces', async () => {
    ...
    const reader = await signJwt({sub:'reader',role:'readonly',typ:'access'}, env.API_SECRET_KEY, 60);
    for (const path of ['/api/config/consumers','/api/config/job-snapshots/job-a']) {
      expect((await app.request(path, {headers:{Authorization:`Bearer ${reader}`}}, env)).status).toBe(403);
    }
    const login = await app.request('/api/auth/login', {method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify({username:'admin',password:'testpassword123'})}, env);
    ...
    expect(body.consumers.map(c => c.status)).toEqual(['observed','unobservable','unobservable']);
    const history = await app.request('/api/config/job-snapshots/job-missing', {headers}, env);
    expect(history.status).toBe(200);
    expect((await history.json() as snapshots.Envelope).consumers.every(c => c.status === 'snapshot_unavailable')).toBe(true);
    expect((await app.request('/api/config/job-snapshots/not%20valid', {headers}, env)).status).toBe(422);
  });
});


it('scheduled cleanup runs without request traffic and surfaces sanitized failure', async () => {
  await snapshots.insertSnapshot(env.OPERATIONS_DB, 'job-expired', {...golden.snapshot, captured_at: '2020-01-01T00:00:00.000Z'} as snapshots.Snapshot);
  await worker.scheduled({} as ScheduledController, env);
  expect((await env.OPERATIONS_DB.prepare('SELECT COUNT(*) AS n FROM ConfigSnapshots').first<{n:number}>())?.n).toBe(0);
});
...
it('a missing acknowledgement read alerts instead of silently downgrading', async () => {
  ...
  const db = {prepare(sql: string) {
    ...
    return {bind(...args: unknown[]) {
      ...
      if (sql.includes('expires_at >')) return {all: async () => {allReads++; throw new Error('secret-canary');}};
      return bound;
    }, all: () => statement.all()};
